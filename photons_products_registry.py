...
class Capability(dictobj.Spec):
    ...
    min_extended_fw = dictobj.NullableField(sb.tuple_spec(sb.integer_spec(), sb.integer_spec()))

    def has_extended_multizone(self, version_major, version_minor):
        if any(thing is None for thing in (self.min_extended_fw, version_major, version_minor)):
            return False

        desired_major, desired_minor = self.min_extended_fw
        return version_major >= desired_major and version_minor >= desired_minor


def capability(name, company, identifier, **kwargs):
    return Capability.FieldSpec().empty_normalise(
        name=name, company=company, identifier=identifier, **kwargs
    )
...
class Capabilities(Enum):
    ...
    LCMV4_A19_WHITE_LV = lc(
        "White 800", "a19_white", has_color=False, min_kelvin=2700, max_kelvin=6500
    )
    LCMV4_A19_WHITE_HV = lc(
        "White 800", "a19_white", has_color=False, min_kelvin=2700, max_kelvin=6500
    )
    LCMV4_BR30_WHITE_LV = lc(
        "White 900 BR30", "br30_white", has_color=False, min_kelvin=2700, max_kelvin=6500
    )
    ...
    LCM2_Z = lc("LIFX Z", "z", has_multizone=True, min_extended_fw=(2, 77))
    ...
    LCM3_MINI_DAY_DUSK = lc(
        "LIFX Mini Day Dusk", "mini_day_dusk", has_color=False, min_kelvin=1500, max_kelvin=4000
    )
    LCM3_MINI_WHITE = lc(
        "LIFX Mini White", "mini_white", has_color=False, min_kelvin=2700, max_kelvin=2700
    )
