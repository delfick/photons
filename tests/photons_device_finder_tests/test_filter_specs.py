...
describe "boolean":
    it "transforms int into a boolean":
        assert boolean().normalise(Meta.empty(), 0) == False
        for i in (1, 20, 100):
            assert boolean().normalise(Meta.empty(), i) == True

    it "transforms a str into boolean":
        for s in ("no", "false", "No", "NO", "False", "FALSE"):
            assert boolean().normalise(Meta.empty(), s) == False

        for s in ("yes", "true", "True", "TRUE", "YES"):
            assert boolean().normalise(Meta.empty(), s) == True

    it "passes through booleans":
        assert boolean().normalise(Meta.empty(), False) == False
        assert boolean().normalise(Meta.empty(), True) == True
