...
describe AsyncTestCase, "Transport":
    ...
    describe "__init__":
        async it "takes in arguments":
            ...
            self.assertIs(transport.session, self.session)
        ...
        async it "has a setup function":
            ...
            self.assertIs(transport.one, one)
            self.assertIs(transport.two, two)
            self.assertIs(transport.three, three)
    ...
    describe "spawn":
        ...
        async it "gets the transport from spawn_transport":
            ...
            self.assertIs(s, self.spawned)
            self.assertEqual(self.called, [("spawn_transport", 10)])
        ...
        async it "re gets the transport was cancelled first time":
            ...
            self.assertIs(s, self.spawned)
            self.assertEqual(self.called, [("spawn_transport", 10), ("spawn_transport", 20)])
        ...
        async it "re gets transport if it's no longer active":
            ...
            with mock.patch.object(self.transport, "is_transport_active", is_transport_active):
                ...
                self.assertEqual(len(is_transport_active.mock_calls), 0)
