...
describe AsyncTestCase, "Communication":
    ...
    describe "add_service":
        ...
        async it "can replace a transport when closing it fails":
            ...
            service = mock.Mock(name="service")
            othertransport = mock.Mock(name="othertransport")
            othertransport.close = asynctest.mock.CoroutineMock(name="close", side_effect=Exception("NOPE"))
    ...
    describe "find_devices":
        async it "uses find_specific_serials":
            ...
            with mock.patch.object(self.communication, "find_specific_serials", find_specific_serials):
    ...
    describe "find_specific_serials":
        ...
        async def assertSpecificSerials(self, serials, found, missing):
            ...
            with mock.patch.object(self.communication, "_find_specific_serials", _find_specific_serials):
                f2, m = await self.communication.find_specific_serials(serials
                    , ignore_lost = ignore_lost
                    , raise_on_none = raise_on_none
                    , broadcast = broadcast
                    , a = a
            ...
            with mock.patch.object(self.communication, "_find_specific_serials", _find_specific_serials):
                f3, m = await self.communication.find_specific_serials(serials
                    , broadcast = broadcast
                    , a = a
                    )
    ...
    describe "private find_specific_serials":
        async it "uses _do_search":
            ...
            with mock.patch.object(self.communication, "_do_search", _do_search):
                self.assertIs(await self.communication._find_specific_serials(serials, timeout=timeout, a=a), found)
        ...
        async it "does not remove lost if ignore_lost":
            ...
            with mock.patch.object(self.communication, "_do_search", _do_search):
                self.assertIs(await self.communication._find_specific_serials(serials, timeout=timeout, ignore_lost=True, a=a), found)
        ...
        async it "complains if none are found and raise_on_none":
            ...
            with self.fuzzyAssertRaisesError(FoundNoDevices):
                with mock.patch.object(self.communication, "_do_search", _do_search):
                    await self.communication._find_specific_serials(None, timeout=timeout, raise_on_none=True, a=a)
        ...
        async it "does not complain if none are found and raise_on_none but non None serials":
            ...
            with mock.patch.object(self.communication, "_do_search", _do_search):
                self.assertIs(await self.communication._find_specific_serials(serials, raise_on_none=True, timeout=timeout, a=a), found)
        ...
        async it "does not complain if none are found if not raise_on_none":
            ...
            with mock.patch.object(self.communication, "_do_search", _do_search):
                self.assertIs(await self.communication._find_specific_serials(None, ignore_lost=True, timeout=timeout, a=a), found)
    ...
    describe "broadcast":
        async it "is a send with a broadcast transport":
            ...
            make_broadcast_transport = asynctest.mock.CoroutineMock(name="make_broadcast_transport", return_value=transport)
    ...
    describe "send":
        ...
        async it "sends":
            ...
            _get_response = asynctest.mock.CoroutineMock(name="_get_response", side_effect=_get_response)
            ...
            with patch_comms, patch_writer, patch_waiter:
                ...
                _transport_for_send.assert_called_once_with(transport_in, packet, original, is_broadcast_in, connect_timeout)
                retry_options_for.assert_called_once_with(original, transport_out)
                FakeWriter.assert_called_once_with(self.communication
                    , transport_out, self.communication.receiver, original, packet, retry_options
                    , did_broadcast = is_broadcast_out
                    , connect_timeout = connect_timeout
                    )
                FakeWaiter.assert_called_once_with(self.communication.stop_fut, writer, retry_options, no_retry=no_retry)
    ...
    describe "private transport_for_send":
        ...
        def maker_mocks(self):
            make_broadcast_transport = asynctest.mock.CoroutineMock(name="make_broadcast_transport", return_value=self.transport)
            choose_transport = asynctest.mock.CoroutineMock(name="choose_transport", return_value=self.transport)

            mbt_patch = mock.patch.object(self.communication, "make_broadcast_transport", make_broadcast_transport)
        ...
        async it "uses make_broadcast_transport if is_broadcast":
            with self.maker_mocks() as (make_broadcast_transport, choose_transport):
                res = await self.communication._transport_for_send(None, self.packet, self.original, True, self.connect_timeout)
        ...
        async it "uses make_broadcast_transport if packet target is None":
            ...
            with self.maker_mocks() as (make_broadcast_transport, choose_transport):
                res = await self.communication._transport_for_send(None, self.packet, self.original, False, self.connect_timeout)
                self.assertEqual(res, (self.transport, True))
        ...
        async it "complains if the serial doesn't exist":
            ...
            with self.fuzzyAssertRaisesError(FailedToFindDevice, serial="d073d5"):
                with self.maker_mocks() as (make_broadcast_transport, choose_transport):
                    await self.communication._transport_for_send(None, self.packet, self.original, False, self.connect_timeout)
        ...
        async it "just spawns transport if one is provided":
            with self.maker_mocks() as (make_broadcast_transport, choose_transport):
                res = await self.communication._transport_for_send(self.transport, self.packet, self.original, False, self.connect_timeout)
                self.assertEqual(res, (self.transport, False))
    ...
    describe "private get_response":
        ...
        async it "can timeout a task":
            ...
            with self.fuzzyAssertRaisesError(TimedOut, "Waiting for reply to a packet", serial=self.serial):
                await t
