...
describe TestCase, "Packet attributes":
    describe "__getitem__":
        ...
        it "uses sb.NotSpecified if there is no value for the key":
            ...
            getitem_spec.assert_called_once_with(T.String, "one", sb.NotSpecified, None, None, True, False, True)
        ...
        it "passes along information provided to getitem_spec":
            ...
            options = dict(
                do_transform=do_transform, parent=parent, serial=serial
                , allow_bitarray=allow_bitarray, unpacking=unpacking
                )
            ...
            getitem_spec.assert_called_once_with(T.String, "one", v, parent, serial, do_transform, allow_bitarray, unpacking)
    ...
    describe "getitem_spec":
        ...
        it "does not transform if do_transform is False":
            ...
            p = P()
            meta = Meta.empty()
            self.assertIs(self.getitem_spec(p, actual, do_transform=False, allow_bitarray=True), self.normalised)
        ...
        it "does not transform if the value from spec is sb.NotSpecified":
            ...
            self.assertIs(self.getitem_spec(p, sb.NotSpecified, do_transform=True, allow_bitarray=True), sb.NotSpecified)
        ...
        it "does not transform if the value from spec is Optional":
            ...
            self.assertIs(self.getitem_spec(p, sb.NotSpecified, do_transform=True, allow_bitarray=True), Optional)
        ...
        it "keeps transformed values as bitarrays if allow_bitarray":
            ...
            p = P()
            meta = Meta.empty()
            self.assertEqual(self.getitem_spec(p, actual, do_transform=True, allow_bitarray=True), bitarray('0000'))
        ...
        it "keeps untransformed values as bitarrays if allow_bitarray":
            ...
            p = P()
            meta = Meta.empty()
            self.assertEqual(self.getitem_spec(p, actual, do_transform=False, allow_bitarray=True), bitarray('0000'))
        ...
        it "does not transform if we are not unpacking":
            ...
            self.assertEqual(self.getitem_spec(p, actual, do_transform=True, allow_bitarray=True), self.normalised)
    ...
    describe "__setitem__":
        ...
        describe "transformation":
            before_each:
                ...
                self.two_typ = mock.Mock(name="two_typ", _transform=sb.NotSpecified, spec=["_transform"])
            ...
            it "does no transformation if the val is sb.NotSpecified":
                ...
                self.assertEqual(list(p.items()), [("one", sb.NotSpecified), ("two", sb.NotSpecified)])
            ...
            it "does transformation for other values":
                ...
                self.assertEqual(list(p.items()), [("one", self.for_packing), ("two", sb.NotSpecified)])
    ...
    describe "_set_group_item":
        describe "setting empty payload":
            ...
            it "complains if we aren't setting bytes or str or bitarray":
                for val in (0, 1, None, True, False, [], [1], {}, {1:2}, lambda: 1, self.P.Payload(), sb.NotSpecified):
                    with self.fuzzyAssertRaisesError(ValueError, "Setting non bytes payload on a packet.+"):
                        self.P()["payload"] = val
        ...
        describe "setting a group as sb.NotSpecified":
            ...
            it "sets all the fields in that group as NotSpecified":
                ...
                p["g"] = sb.NotSpecified
                self.assertEqual(sorted(p.items()), sorted([("one", sb.NotSpecified), ("two", sb.NotSpecified)]))
        ...
        describe "setting a group from an instance of that group":
            it "does a direct copy without going through transformation or specs":
                ...
                two_typ = mock.Mock(name="two_typ"
        ...
        describe "setting a group from not an instance of that group":
            ...
            it "complains if the value does not have items":
                ...
                p = self.P()
                with self.fuzzyAssertRaisesError(ValueError, "Setting a group on a packet must be done with a value that has an items\(\) method.+"):
                    p["g"] = val

            it "sets values from items on the val":
                val = mock.Mock(name='val', spec=["items"])
                val.items.return_value = [("one", self.for_user), ("two", "d073d5"), ("three", "not given")]

                p = self.P()
                self.assertEqual(sorted(p.items()), sorted([("one", sb.NotSpecified), ("two", sb.NotSpecified)]))

                p["g"] = val
                self.assertEqual(sorted(p.items()), sorted([("one", self.for_packing), ("two", "d073d5")]))
