"""
This module provides an object for finding serials based on some filter.

And a class for watching the network continuously asking devices for information
so that you can query the daemon for information.

Valid Filters
-------------

You create a filter by using the Filter class:
...
You can also create a SpecialReference object by calling any of these messages
on ``photons_control.device_finder.DeviceFinder``.
...
To create a special reference, you use
``photons_control.device_finder.DeviceFinder``. This has the same classmethods
as ``Filter`` for creating a DeviceFinder. Or you can create a ``DeviceFinder``
by instantiating it with a ``fltr`` you have already created.
...
.. code-block:: python
    ...
    from photons_control.device_finder import DeviceFinderDaemon
...
By default the daemon has a in flight message limit of 30, you can change this by
either specifying ``limit=<integer>`` or ``limit=asyncio.Semaphore(<integer>)``. If you
specify just a number, a semaphore is made for you.
"""
