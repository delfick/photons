...
describe "SetCleanConfig":
    ...
    async it "sets the default config", sender:
        ...
        await self.run_and_compare(
            sender, SetCleanConfig(indication=True, duration=3600), expected=expected
        )
...
describe "ChangeCleanCycle":
    ...
    async it "starts a clean cycle", sender:
        ...
        await self.run_and_compare(
            sender, ChangeCleanCycle(enable=True, duration=7200), expected=expected
        )
    ...
    async it "stops a clean cycle", sender:
        ...
        await self.run_and_compare(
            sender, ChangeCleanCycle(enable=False, duration=0), expected=expected
        )
