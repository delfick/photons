...
def incoming_event(device, io):
    return Events.INCOMING(device, io, pkt=DeviceMessages.GetPower(source=20, sequence=34, target=device.serial))
    ...
    class TestProcessRequest:
        ...
        class TestLostRequest:
            async def test_it_causes_a_message_to_return_None(self, fltr, device, make_incoming):
                await fltr.process_request(make_incoming(device.serial, DeviceMessages.GetPower)) is True

                with fltr.lost_request(DeviceMessages.GetPower, DeviceMessages.SetPower):
                    await fltr.process_request(make_incoming(device.serial, DeviceMessages.GetPower)) is None
                    await fltr.process_request(make_incoming("d073d5656565", DeviceMessages.GetPower)) is None

                    await fltr.process_request(make_incoming(device.serial, DeviceMessages.SetPower)) is None

                    await fltr.process_request(make_incoming(device.serial, DeviceMessages.GetLabel)) is True

                    with fltr.lost_request(DeviceMessages.SetGroup):
                        await fltr.process_request(make_incoming(device.serial, DeviceMessages.GetPower)) is None
                        await fltr.process_request(make_incoming(device.serial, DeviceMessages.GetGroup)) is None

                    await fltr.process_request(make_incoming(device.serial, DeviceMessages.GetGroup)) is True
    ...
    class TestOutgoing:
        ...
        def make_incoming(self, device, io):
            def make_incoming(kls):
                return Events.INCOMING(device, io, pkt=kls(source=200, sequence=300, target=device.serial))
        ...
        class TestLostReplies:
            ...
            async def test_it_can_ignore_replies_that_themselves_are_a_particular_class(self, make_incoming, outgoing, fltr):
                ...
                with fltr.lost_replies(DeviceMessages.StatePower):
                    ...
                    assert await outgoing(ack, make_incoming(DeviceMessages.SetPower)) == [ack]
                    assert await outgoing(reply1, make_incoming(DeviceMessages.SetPower)) == []
                    assert await outgoing(reply2, make_incoming(DeviceMessages.SetPower)) == [reply2]
